let oldArticles = document.getElementById("oldArticles");

// getting products implementation below
class MainOldPosts {
  async getPosts(){
    try {
      let result = await fetch("mainOlder.json");
...
class MainOldUI {
  loadAllMainOldPosts(posts){
    ...
    oldArticles.innerHTML = indexPostResult;
  }
}

// DOM load event 
document.addEventListener("DOMContentLoaded", ()=>{
  const ui = new MainOldUI();
  const posts = new MainOldPosts();

  //get product Posts
  posts.getPosts().then(posts => {
    ui.loadAllMainOldPosts(posts);
  })
})
